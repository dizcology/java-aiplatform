...
public class UpdateExecutionSample {

  public static void main(String[] args) throws IOException {
    // TODO(developer): Replace these variables before running the sample.
    String name = "NAME";
    String displayName = "DISPLAY_NAME";
    updateExecutionSample(name, displayName);
  }

  static void updateExecutionSample(String name, String displayName) throws IOException {
    ...
    try (MetadataServiceClient client = MetadataServiceClient.create(settings)) {
      Execution execution =
          Execution.newBuilder().setName(name).setDisplayName(displayName).build();
      FieldMask updateMask = FieldMask.newBuilder().addPaths("display_name").build();
      Execution response = client.updateExecution(execution, updateMask);

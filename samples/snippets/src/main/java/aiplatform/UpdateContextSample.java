...
// [START aiplatform_update_context_sample]
...
public class UpdateContextSample {

  public static void main(String[] args) throws IOException {
    // TODO(developer): Replace these variables before running the sample.
    String name = "NAME";
    String displayName = "DISPLAY_NAME";
    updateContextSample(name, displayName);
  }

  static void updateContextSample(String name, String displayName) throws IOException {
    ...
    try (MetadataServiceClient client = MetadataServiceClient.create(settings)) {
      Context context = Context.newBuilder().setName(name).setDisplayName(displayName).build();
      FieldMask updateMask = FieldMask.newBuilder().addPaths("display_name").build();
      Context response = client.updateContext(context, updateMask);
      System.out.format("response: %s\n", response);
    }
  }
}

// [END aiplatform_update_context_sample]

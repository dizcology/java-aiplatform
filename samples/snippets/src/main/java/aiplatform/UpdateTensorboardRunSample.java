...
public class UpdateTensorboardRunSample {

  public static void main(String[] args) throws IOException {
    // TODO(developer): Replace these variables before running the sample.
    String name = "NAME";
    String displayName = "DISPLAY_NAME";
    updateTensorboardRunSample(name, displayName);
  }

  static void updateTensorboardRunSample(String name, String displayName) throws IOException {
    ...
    try (TensorboardServiceClient client = TensorboardServiceClient.create(settings)) {
      TensorboardRun tensorboardRun =
          TensorboardRun.newBuilder().setName(name).setDisplayName(displayName).build();
      FieldMask updateMask = FieldMask.newBuilder().addPaths("display_name").build();
      TensorboardRun response = client.updateTensorboardRun(tensorboardRun, updateMask);

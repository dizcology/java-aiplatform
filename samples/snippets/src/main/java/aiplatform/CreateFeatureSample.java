...
public class CreateFeatureSample {
  ...
  public static void main(String[] args)
    ...
    createFeatureSample(project, location, featurestoreId, entityTypeId, description);
  }
  ...
  static void createFeatureSample(
    ...
    try (FeaturestoreServiceClient client = FeaturestoreServiceClient.create(settings)) {
      Feature feature = Feature.newBuilder().setDescription(description).build();
      EntityTypeName parent = EntityTypeName.of(project, location, featurestoreId, entityTypeId);
      OperationFuture<Feature, CreateFeatureOperationMetadata> response =
          client.createFeatureAsync(parent, feature);
      ...
      Feature createdFeature = response.get();
      System.out.format("createdFeature: %s\n", createdFeature);
    }
  }
}

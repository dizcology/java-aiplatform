...
public class UpdateFeatureSample {
  ...
  public static void main(String[] args) throws IOException {
    ...
    updateFeatureSample(name, displayName);
  }

  static void updateFeatureSample(String name, String displayName) throws IOException {
    ...
    try (FeaturestoreServiceClient client = FeaturestoreServiceClient.create(settings)) {
      Feature feature = Feature.newBuilder().setName(name).setDisplayName(displayName).build();
      FieldMask updateMask = FieldMask.newBuilder().addPaths("display_name").build();
      Feature response = client.updateFeature(feature, updateMask);

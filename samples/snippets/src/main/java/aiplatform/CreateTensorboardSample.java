...
public class CreateTensorboardSample {
  ...
  public static void main(String[] args)
    ...
    createTensorboardSample(project, location, tensorboardId, displayName);
  }

  static void createTensorboardSample(
      String project, String location, String tensorboardId, String displayName)
      throws IOException, ExecutionException, InterruptedException {
    ...
    try (TensorboardServiceClient client = TensorboardServiceClient.create(settings)) {
      Tensorboard tensorboard = Tensorboard.newBuilder().setDisplayName(displayName).build();
      TensorboardName parent = TensorboardName.of(project, location, tensorboardId);
      OperationFuture<Tensorboard, CreateTensorboardOperationMetadata> response =
          client.createTensorboardAsync(parent, tensorboard);
      ...
      Tensorboard createdTensorboard = response.get();
      System.out.format("createdTensorboard: %s\n", createdTensorboard);
    }
  }
}
